...
function addToCart(productName, productPrice, productImage) {
    ...
    // Update the cart UI with new product
    updateCartUI();

    // Save the cart in sessionStorage so it can be used on z3.html
...
async function fetchProducts() {
    try {
        const response = await fetch('p.json'); // Ensure the path is correct
...
function renderProducts(products) {
    ...
    document.querySelectorAll('.tt-product-adc-btn').forEach(button => {
        button.addEventListener('click', function (event) {
            ...
            // Perform token validation asynchronously in the background
            checkToken();
...
async function fetchAndRenderProducts({ category = null, saleStatus = null, sortOption = null } = {}) {
    ...
    // Render the filtered/sorted products
    renderProducts(products);
}

// Initialize the page by fetching and rendering products
